...
void Image::Render() {
  ...
  glDrawPixels(width, height, bytes_per_pixel == 3 ? GL_RGB : GL_RGBA,
...
void Image::DrawRect(int x, int y, int w, int h, const Color &borderColor, int borderWidth, bool isFilled, const Color &fillColor) {
  ...
  if (isFilled == true) {
    ...
    position and then we paint all the y's in that x coordinate*/
    for (int i = 0; i < w; i++) {
      for (int j = 0; j < h; j++) {
  ...
  for (int i = 0; i < borderWidth; i++) {
    ...
    DrawLineDDA(x + i, y + i, x + w - 1 - i, y + i,
                borderColor); // this is the top line
    DrawLineDDA(x + i, y + i, x + i, y + h - 1 - i, borderColor); // Left
    DrawLineDDA(x + i, y + h - 1 - i, x + w - 1 - i, y + h - 1 - i,
                borderColor); // Bottom
    DrawLineDDA(x + w - 1 - i, y + i, x + w - 1 - i, y + h - 1 - i,
                borderColor); // Right
  }
}
...
void Image::ScanLineDDA(int x0, int y0, int x1, int y1, std::vector<Cell> &table) {
  ...
  float x = (float)x0;
  float y = (float)y0;
  /*Now we go to the direction that the vector tells us until we get to the
   * final position we update the table*/
  for (int i = 0; i <= d; i++) {
    int iX = (int)floor(x);
    int iY = (int)floor(y);
...
Image Image::GetArea(unsigned int start_x, unsigned int start_y,
...
bool Image::LoadPNG(const char *filename, bool flip_y) {
  ...
  unsigned int originalBytesPerPixel =
...
bool Image::LoadTGA(const char *filename, bool flip_y) {
  ...
  if (tgainfo->data == NULL ||
      fread(tgainfo->data, 1, imageSize, file) != imageSize) {
    std::cerr << "--- Failed to load file: " << sfullPath.c_str() << std::endl;
  ...
  for (unsigned int y = 0; y < height; ++y) {
    for (unsigned int x = 0; x < width; ++x) {
      ...
      if ((pos < imageSize) && (pos + 1 < imageSize) && (pos + 2 < imageSize))
        SetPixelUnsafe(x, height - y - 1,
                       Color(tgainfo->data[pos + 2], tgainfo->data[pos + 1],
                             tgainfo->data[pos]));
    }
  }
